...
object Util {
  ...
  def removeUnknowns(inputFilename: String, outputFilename: String) {
    if (null == inputFilename || null == outputFilename) { println("expect input file as parameter"); }
    else {
      val unknownKeyword = "PAUNKNOWN"
      assert(new File(inputFilename).exists(), "File " + inputFilename + " does not exist")
     ...
     val out =      new FileWriter(outputFilename)
		...
		{
		  if(!(line contains unknownKeyword))
		    out.write(line+"\n")
         ...
         println("removed "+unknownKeyword)
